...
- [서비스 소개](${SITE_URL}/about): 운영 주체, 만든 이유, 데이터 처리 원칙
- [문의하기](${SITE_URL}/contact): 측정값 오류 제보와 내용 정정 창구
- [이용약관](${SITE_URL}/terms): 이용 조건과 측정값의 성격·한계
- [측정 방법론](${SITE_URL}/methodology): 측정 알고리즘 공개 — Cloudflare 엣지에
  HTTPS 다중 스트림 연결, 15초 상한, 최소 8초 측정 후 안정 시 조기 종료.
  핑은 다운로드 시작 전에 순차 측정한다(첫 1회는 워밍업으로 버린다).
...
- 측정 방식: Cloudflare 엣지에 HTTPS 다중 스트림을 연결해 측정한다. 다운로드는 최대 15초
  상한이며 최소 8초 측정 후 최근 2초 구간과 직전 2초 구간의 속도 중앙값이 5% 이내로 안정되고 하락 중이 아닐 때만 조기 종료한다. 최종값은
  마지막 4초 구간의 중앙값이다. 핑은 다운로드 시작 전에 워밍업 1회를 버리고 순차로 측정하며, 지터는 연속한
  RTT 사이 차이의 평균이다. 상세는 ${SITE_URL}/methodology 참조.
