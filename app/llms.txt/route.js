...
- [측정 방법론](${SITE_URL}/methodology): 측정 알고리즘 공개 — Cloudflare 엣지에
  HTTPS 다중 스트림 연결, 15초 상한, 최소 6초 측정 후 안정 시 조기 종료.
  핑은 다운로드 시작 전에 순차 측정한다.
...
- 측정 방식: Cloudflare 엣지에 HTTPS 다중 스트림을 연결해 측정한다. 다운로드는 최대 15초
  상한이며 최소 6초 측정 후 속도가 안정되고 하락 중이 아닐 때만 조기 종료한다. 최종값은
  마지막 4초 구간의 중앙값이다. 핑은 다운로드 시작 전에 순차로 측정하며, 지터는 연속한
