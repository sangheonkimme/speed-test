...
export async function measureDownload(onProgress, onLoadedLatency) {
  ...
  async function runStream() {
    ...
    while (!aborted && now() - start < CFG.dlMaxDurationMs) {
      const size = CFG.dlChunkSizes[Math.min(sizeIdx, CFG.dlChunkSizes.length - 1)];
      sizeIdx++;
  ...
  const samplerDone = new Promise((resolve) => {
    ...
    const iv = setInterval(() => {
      ...
      const finish = () => { clearInterval(iv); resolve(); };
  ...
  const stable = speedSamples.slice(Math.floor(speedSamples.length * 0.4)).map(s => s.mbps);
  return {
...
export async function measureUpload(onProgress) {
  ...
  let aborted = false;
  const speedSamples = [];
  const payload = new Uint8Array(CFG.ulChunkSize);
  ...
  const samplerDone = new Promise((resolve) => {
    ...
    const iv = setInterval(() => {
      ...
      if (onProgress && speedSamples.length) {
        onProgress({ mbps: median(speedSamples.slice(-6)), elapsedMs: t - start });
      }
      if (t - start >= CFG.ulMaxDurationMs) { clearInterval(iv); resolve(); }
    }, CFG.sampleIntervalMs);
  });
  ...
  return { mbps: Math.round(median(stable) * 10) / 10, durationMs: Math.round(now() - start) };
}
...
export async function detectEnvironment() {
  ...
  if (conn && conn.type) {
    connType = { wifi: 'WiFi', cellular: '셀룰러', ethernet: '유선' }[conn.type] || conn.type;
  } else {
    connType = isMobile ? 'WiFi/셀룰러' : 'WiFi/유선';
  }

  // IP 기반 ISP·지역 (시·군·구 수준, 정밀 위치 수집 안 함)
  let isp = null, region = null;
  try {
    ...
    if (org.includes('korea telecom') || org.includes('kt ') || org === 'kt' || org.includes('kt corporation')) isp = 'KT';
    else if (org.includes('sk broadband') || org.includes('skb') || org.includes('sk telecom')) isp = 'SK브로드밴드';
    else if (org.includes('lg') || org.includes('powercomm') || org.includes('dacom')) isp = 'LG유플러스';
