...
export function verdict(dl, isMobile = false) {
  ...
  if (dl >= ok) return { text: '정상', color: '#00BF40', bg: 'rgba(0,191,64,0.12)' };
  if (dl >= slow) return { text: '느림', color: '#FF9F00', bg: 'rgba(255,159,0,0.14)' };
  return { text: '매우 느림', color: '#FF4242', bg: 'rgba(255,66,66,0.12)' };
}

// 등급 라벨 (FR-6) — 실사용 체감 기준 (넷플릭스 4K ≈ 15~25Mbps)
export function grade(dl) {
  if (dl >= 500) return ['기가급 속도예요 — 뭘 해도 여유', '대용량 다운로드·동시 사용까지 쾌적해요'];
  if (dl >= 100) return ['4K 스트리밍도 여유로워요', '여러 기기 동시 사용까지 쾌적해요'];
  if (dl >= 50) return ['4K 스트리밍까지 무난해요', '여러 기기가 동시에 4K를 보면 빠듯할 수 있어요'];
  if (dl >= 20) return ['HD 시청은 충분해요', '4K·대용량 다운로드는 답답할 수 있어요'];
  if (dl >= 5) return ['웹서핑 위주로 쓸 만해요', '고화질 영상은 버퍼링이 생길 수 있어요'];
  return ['기본 사용도 버거워요', '웹서핑·영상까지 자주 끊길 수 있어요'];
}

// 용도별 적합도 (FR-6) — 서비스 권장 대역폭 기준
export function usages(dl, ping, jitter) {
  const L = (lv) => lv === 2 ? { level: '좋음', tone: 'ok' }
    : lv === 1 ? { level: '보통', tone: 'warn' }
      : { level: '나쁨', tone: 'bad' };
...
export function commerce(v, r, env) {
  if (v.text === '정상') return [];
  const list = [];
  if (env && /WiFi|셀룰러/i.test(env.connType || '')) list.push({
    id: 'mesh', tag: '쿠팡 파트너스 · 제휴 링크', icon: 'wifi',
    title: '방마다 느리다면 메시 와이파이',
    desc: 'Wi-Fi 환경에서 속도가 낮게 측정됐어요',
  });
  if (r.ping >= 30 || r.jitter >= 8) list.push({
    id: 'gaming', tag: '제휴 링크', icon: 'fire',
